...
```
  ...
  └─────────────┘                        ┌────────▼────────┐
                                         │ STEP 3           │
                                         │ CV PARSING &     │ ◀── auto-runs on
                                         │ ENTITY EXTRACT   │     upload (Gemini)
                                         └────────┬────────┘
                                                  │ saves to DB
                                                  │ (candidate_profiles)
                                                  │
                                         ── GATE: user action required ──
                                         POST /api/cv-research/:id
                                                  │
                                         ┌────────▼────────┐
                                         │ STEP 4           │
  ...
  ── Web Upload API Flow ──────────────────────────────────────────
  POST /api/cv-upload     → saves file, fires Step 3 in background
  POST /api/cv-research/:id → triggers Steps 4-7 in background
  ────────────────────────────────────────────────────────────────
```
