...
describe("callTinyFish", () => {
	...
	it("POSTs { url, goal, browser_profile } to /v1/automation/run-sse", async () => {
		const mockFetch = vi.fn().mockResolvedValue(
			mockSseResponse([
				...
				{ type: "COMPLETE", run_id: "run-1", status: "COMPLETED", result: { ok: true } },
			]),
		);
		vi.stubGlobal("fetch", mockFetch);

		await callTinyFish({ url: "https://example.com", goal: "Extract data", browser_profile: "lite" });

		const [url, init] = mockFetch.mock.calls[0] as [string, RequestInit];
		expect(url).toBe("https://agent.tinyfish.ai/v1/automation/run-sse");
		expect(init.method).toBe("POST");
		const body = JSON.parse(init.body as string);
		expect(body).toEqual({ url: "https://example.com", goal: "Extract data", browser_profile: "lite" });
	});
	...
	it("returns run_id and result from the COMPLETE event", async () => {
		vi.stubGlobal("fetch", vi.fn().mockResolvedValue(
			mockSseResponse([
				...
				{ type: "COMPLETE", run_id: "run-42", status: "COMPLETED", result: { username: "alice" } },
			]),
		));

		const { run_id, result } = await callTinyFish({ url: "https://github.com/alice", goal: "get profile" });
	...
	it("throws TinyFishError on non-2xx HTTP response", async () => {
		...
		await expect(callTinyFish({ url: "https://example.com", goal: "test" }))
	...
	it("throws TinyFishError when COMPLETE event has status FAILED", async () => {
		vi.stubGlobal("fetch", vi.fn().mockResolvedValue(
			mockSseResponse([
				...
				{ type: "COMPLETE", run_id: "run-bad", status: "FAILED", error: { message: "Navigation timeout" } },
			]),
		));
	...
	it("forwards AbortSignal to fetch", async () => {
		...
		await callTinyFish({ url: "https://example.com", goal: "test" }, controller.signal);
